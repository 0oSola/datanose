...
(function(){
	...
	//test
	var datatest = '{"chart_name":["增加量","激活量"],"y_name":"人数","x_name":["12/14","12/15","12/16","12/17","12/18","12/19","12/20"],"chart_data":[[3, 4, 3, 5, 4, 10, 12],[1, 3, 4, 3, 3, 5, 4]],"data_list":[["日期","设备","新增账户","新增设备"],["2015-01-01","11","0","21"],["2012-01-01","11","10","21"],["2018-01-01","1","0","2"]],"totalCount":100,"error_code":0,"error_message":""}';
	//end test
	
	var pageSize = 10;
	var colorLine2 = ["#14ADC4","#7cb5ec","#e55555"];
	...
	$(function(){
		...
		//init
		dataLoader.loadList(".online-content");
		
		eventsManager.searchData();
		eventsManager.normChange();
		...
		viewRender.addSeriesChart();
	});
	
	
	var viewRender = {
		/*清除数据组*/
		cleanSeriesChart:function(){
			$(".remove-compare").on("click",function(){
				var chart = $("#"+$(this).parent().find(".chart-content").attr("id")).highcharts();
		...
		/*图表添加数据组*/
		addSeriesChart:function(){
			$(".add-compare").on("click",function(){
				var chart = $("#"+$(this).parent().find(".chart-content").attr("id")).highcharts();
				chart.addSeries({
					data: [6.4, 4.1, 5.6, 4.4, 2.9, 7.5, 6.4],
					color:colorLine2[3],
					name:"新增1"
				});
			})
	...
	var eventsManager = {
		...
		normChange:function(){
			...
			$(".menu-item").on("click",function(){
				...
				if(type=="content-hide"){
					$(this).find(".content-hide").css("display","none");
					$(this).find(".content-show").css("display","block");
					$(this).attr("data-type","content-show");
					$("#"+target).removeClass("open");
					$("#"+target).find(".realTime-content").css("display","none");
					$(this).height($("#"+target).height());
				}else if(type=="content-show"){
					$(this).find(".content-hide").css("display","block");
					$(this).find(".content-show").css("display","none");
					$(this).attr("data-type","content-hide");
					$("#"+target).addClass("open");
					$("#"+target).find(".realTime-content").css("display","block");
					$(this).height($("#"+target).height());
				}
			});
		},
