...
(function(){
	...
	//test
	var datatest = '{"chart_type":"area","chart_name":["增加量","激活量"],"y_name":"人数","x_name":["12/14","12/15","12/16","12/17","12/18","12/19","12/20"],"chart_data":[[3, 4, 3, 5, 4, 10, 12],[1, 3, 4, 3, 3, 5, 4]],"data_list":[["日期","设备","新增账户","新增设备"],["2015-01-01","11","0","21"],["2012-01-01","13","10","21"],["2018-01-01","1","0","2"]],"totalCount":100,"error_code":0,"error_message":""}';
	//end test
		...
		renderList:function(target,data){
			...
			data.forEach(function(ele,idx){
				if(idx==0){
					...
					//添加详情
					target.find(".table-title").append('<th>详情</th>');
				}else{
					...
					tpl.append('<td><a class="itemDetail" data-target="'+ele[1]+'">查看</a></td>');
					if(idx%2==0){
						tpl.addClass("odd");
					}else{
						tpl.addClass("even");
	...
	var eventsManager = {
		...
		//查看详情
		itemDetail:function(){
			$(".itemDetail").off("click");
			$(".itemDetail").on("click",function(){
				var targetItem = $(this).attr("data-target");
				$("#chart-tab3").slideToggle("slow");
				$("#item-list").slideToggle("slow");
			})
			
			$(".hideItemDetail").off("click");
			$(".hideItemDetail").on("click",function(){
				$("#chart-tab3").slideToggle("slow");
				$("#item-list").slideToggle("slow");
			});
		...
		typeSearch:function(){
			...
			$(".date-type").on("click",function(){
				...
				dataLoader.loadList(".itemList-content");
